...
- **The META shard shares `CFReplData`.** `MetaShardID = 1` (control.go) is **below** `FirstDataShard` (=2) and persists into the same CF: its range-directory rows are `be8(1)|subData|<rangeStart>` (meta.go) and its applied index is `be8(1)|subMeta|…`. `ExportLogical` scans the whole CF, so every backup contains them. On re-shard these must be **dropped by shard id, before `RerouteSuffix`** — decode `be8(key[:8])` and skip any row with `shardID < FirstDataShard` (the target cluster rebuilds its own meta directory + applied index at bootstrap). Critically, an N=1 source's initial range `[-inf,+inf)` is `be8(1)|subData` with an EMPTY routing body, so feeding it to `RerouteSuffix` would error and abort the whole restore — the shard-id guard prevents that. (`RerouteSuffix` only sees the suffix, so the shard-id drop is the caller's job.)
