...
- **Access to the engine:** `*collections.Service` holds `cols *Collections`, `self membership.Member`, `tier *tierStatus` (carries `*Manager`). Add a `WithBackup(coord *backup.Coordinator)` option mirroring `WithTierStatus`. The `*Manager` is the access point for all shards + the meta shard (`MetaShardID`), `proposeRaw` (manager.go:278), reads via `Manager.Read(ctx, shardID, query, linearizable)`, and leadership via `LeaderID(shardID) (uint64, bool)` / `IsLeader` (NOT `GetLeaderID`, which is dragonboat's). Roster via the gossip `svc`/`peersFn` already used by `NewRPCForwarder(peersFn)`.
...
- **gRPC server obj:** `grpcDataSrv` built by `grpcsrv.New(...)` (~main.go:557). `rpcopts.Handler()` supplies shared Connect handler opts. `connectToGRPC` (grpcsrv/errcode.go), `collErr` (collections/service.go:85).
...
- [ ] **Step 2:** implement `agent.go`: `Prepare` (advance HLC past `T` via the node's clock, pin `LocalStore.Snapshot()`, return `{globalSeq, heldRangeSummary}`) and `Export` (run `ExportLogical(...)` filtered to the assignment + `CheckpointToObjectStore` if physical plane; write the per-node sub-manifest; report progress via a callback). Reuse Phase 2 `ExportLogical`. **Note (review):** `ExportLogical` takes a `storage.LocalStore`, but `LocalStore.Snapshot()` returns the narrower `storage.Snapshot` (Get/Scan/Close only). Either add a thin `LocalStore` adapter wrapping the pinned `Snapshot` (Get/Scan delegate to the snapshot; mutating methods unused on the export path), or pass the live store and rely on the `captureMs`/`Version ≤ T` ceiling for the AP-tier cut. Pick the adapter for a clean consistent view; state the choice in the task.
