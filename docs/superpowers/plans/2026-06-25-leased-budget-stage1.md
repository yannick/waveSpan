...
| `internal/collections/command.go` | `ErrNoCapacity`, `ErrBudgetExists`, `ErrLeaseUnknown` (errors live here, not in a separate errors.go) | Modify |
...
Budget keys live under the shard `subData` space alongside collections, using new sub-scope bytes (the existing scope bytes are `scopeCard 0x00 / scopeElem 0x01 / scopeZPtr 0x02 / scopeType 0x03` in `statemachine.go:33-40`; `typeKey` already stores the type header, so a budget pool reuses `scopeType` for WRONGTYPE checks and adds its own pool/lease scopes):
...
// Budget sub-scope bytes (continue the scope* set in statemachine.go:33-40).
const (
	scopeBudPool  byte = 0x05 // the pool record
	scopeBudLease byte = 0x06 // <leaseID> -> leaseRec
)

// Budget modes (Stage 1 ships STRICT only; modeRelaxed reserved for Stage 4).
const (
	modeStrict uint8 = 1
	modeRelaxed uint8 = 2
)
...
> Confirm the exact `storage.StoreOp` field names (`CF`, `Key`, `Value`, `Delete`) and the CF constant used by `setFieldVal` in `hincr.go:33` — copy whatever it uses (the explore notes `CFReplData`). If the delete flag differs, match `clearTTL` in `ttl.go`.
...
> **Stage-1 idempotency limit (documented, by design):** grant idempotency holds only while the lease row
> exists. After `BudgetReturn` deletes it, a late duplicate `BudgetGrant(sameLeaseID)` would grant fresh
> quantity (no tombstone yet; the ring may have evicted). Callers must not reuse a `leaseID` after return.
> Stage 3 adds settled-lease tombstones (spec §6.3) to close the full dedup window.

Command field mapping (reuse `command`/`item` from `command.go:92-106`, no struct changes; `Idem` empty for all):
...
```go
// applyBudInit creates a pool. Returns budExists sentinel (not an error) if one already exists.
func (u *updateCtx) applyBudInit(c command) (ProposeResult, error) {
	...
	k := c.Items[0].Key
	cap := getI64(k[1:])
	p := poolRec{Cap: cap, Available: cap, LeasedOut: 0, Spent: 0, Epoch: 1, Mode: k[0], Rate: getI64(k[9:]), Burst: getI64(k[17:])}
...
func (u *updateCtx) applyBudGrant(c command) (ProposeResult, error) {
	...
	if !found {
		return ProposeResult{Data: budNoLease}, nil // no such budget
	}
	grant := amount
...
> The budget apply functions return `(ProposeResult, error)` directly, so the dispatch is a plain `return u.applyBudGrant(c)` — do **not** adapt to the `(int64,[]byte,error)` HIncr shape. Budget ops carry **no** `c.Idem`, so they are NOT routed through the generic dedup ring in `applyOne` (`statemachine.go:266-276`) — idempotency is enforced inside the apply functions (durable lease-row check for grant; `max`-fold for report; lease-absence for return). Insert this `case` block in the `applyOne` `switch` alongside the existing op cases; place it before the generic `applyCommand` fallthrough.
>
> Note: `decodeCommandInto` rejects ops where `typeForOp(op)==0` (`command.go:211-213`), so the four budget ops are undecodable until Task 1's `typeForOp` cases land — Task 1 is a hard prerequisite for any end-to-end apply here.
>
> Behavior note: a grant against an **uninitialized** budget returns `budNoLease` (→ `ErrLeaseUnknown`), NOT WRONGTYPE. WRONGTYPE is only for a budget op hitting a set/hash/zset collection (the `typeOf` guard below). Don't "fix" the test to expect WRONGTYPE for grant-before-define.
...
## Task 6: Read path — `budStatQuery` Lookup

**Files:**
- Modify: `internal/collections/statemachine.go` (Lookup query dispatch, lines 470-574)
- Modify: `internal/collections/budget.go`
- Test: `internal/collections/budget_test.go`

`BudgetStat` reads the pool via the existing Lookup mechanism (like `cardQuery`/`hGetQuery`). Reads do not affect safety — grants are the only safety-critical path.
...
In `statemachine.go` `Lookup`, add the case (match the snapshot-getter helper name actually used by `cardQuery`, e.g. `s.getDataSnap` or reading from the passed snapshot directly):
...
git commit -m "feat(collections): BudgetStat Lookup query"
```
...
func newTestSM(t *testing.T) *shardSM {
	...
	return &shardSM{baseSM: baseSM{store: mem, shardID: 2, prefix: shardPrefix(2)}} // match base_sm.go prefix scheme
}
...
> Inspect `hincr_test.go` and the `Update` signature to wire `applySingleForTest` correctly — it should construct the overlay, call `applyOne`, and flush via `BatchRC` so the next `mustApply`/`Lookup` sees committed state. If the package already exposes a single-entry test seam, use it; otherwise add a small unexported `applyForTest` in `statemachine.go` guarded by being unexported (no production callers).
...
**Files:**
- Modify: `internal/collections/collections.go` (mirror `HIncrBy` at 232-246)
- Modify: `internal/collections/errors.go`
...
- [ ] **Step 3: Implement the typed API** in `collections.go` (mirror `HIncrBy`'s `proposeCmd` usage at 235-246):

```go
// BudgetDefine creates a STRICT escrow pool with the given cap (micro-units).
func (c *Collections) BudgetDefine(ctx context.Context, ns, coll []byte, cap int64, mode uint8) error {
	...
	if string(data) == string(budExists) {
...
func (c *Collections) BudgetGrant(ctx context.Context, ns, coll, holder []byte, amount int64, leaseID []byte) (int64, error) {
	...
	case string(budNoLease):
		return 0, ErrLeaseUnknown // budget does not exist
	}
	return int64(n), nil
}
...
var (
	...
	ErrLeaseUnknown = errors.New("collections: lease or budget unknown")
)
```
...
```bash
git add internal/collections/collections.go internal/collections/errors.go internal/collections/budget_test.go
...
**Files:**
- Modify: `internal/collections/service.go` (mirror `HIncrBy` handler at 156-229; error mapping at 78-91)
- Modify: `internal/grpcsrv/collections.go` (mirror the delegation pattern) — or a new `internal/grpcsrv/budget.go` if `BudgetService` is a separate gRPC service registration
...
- [ ] **Step 3: Implement the Connect handlers** in `service.go`, each building the typed call and mapping errors. Extend the error switch (lines 78-91):
...
- [ ] **Step 2: Property/fuzz the conservation invariant** — add a randomized test that interleaves define/grant/report/return with random amounts and leaseIDs across several holders and asserts `assertInv` after every op and that `Spent ≤ Cap` always.
...
func TestBudgetConservationFuzz(t *testing.T) {
	...
	for i := 0; i < 5000; i++ {
		// ... random op; after each: p,_,_ := u.getPool(ns,coll); assertInv(t, p)
	}
}
```
...
- **Match conventions, don't invent:** the exact `storage.StoreOp` fields, the CF constant, the `errShortCommand` name, the snapshot-getter used by `cardQuery`, and the single-entry apply seam must be read from the actual code and matched — placeholders above (`getDataSnap`, `applySingleForTest`, `shardPrefix`) are named by intent; confirm/replace with the real symbols.
