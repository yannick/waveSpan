...
func ExportLogical(src storage.LocalStore, store ObjectStore, keyPrefix string, reg *Registry, captureMs int64, sel Selector) (*NodeManifest, error) {
	...
	// design — strict-dropping them would lose data). CFKVMeta is exported VERBATIM (authoritative,
	// preserving the latest pointer's SiblingVersions / conflict state); restore runs a repair pass that
	// repoints only the rare latest pointer whose winner version was after T (recordstore.RepairCutMeta).
	cutKV := captureMs > 0
