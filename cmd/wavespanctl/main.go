// Command wavespanctl is the WaveSpan admin/client CLI. It is built on the Go SDK
// (github.com/yannick/wavespan/sdk/go) and covers every data-plane operation — KV, Collections
// (set/hash/zset + list/tier/bulk-remove), Cypher graph queries, Vector search, leased budgets
// (controller + node-side holder), and cluster backups — plus cluster membership (admin port) and an
// offline storage snapshot/restore tool that operates directly on a wavesdb directory.
//
// Usage (global flags come BEFORE the group/op — the stdlib flag parser stops at the first positional):
//
//	wavespanctl [global-flags] <group> <op> [args...]
//
//	-addr host:port   node data-port address              (default localhost:7800)
//	-admin host:port  node admin-port address             (default localhost:7900)
...
// Examples:
//
//	wavespanctl kv put greeting "hello, wavespan"
//	wavespanctl -lin coll ls
//	wavespanctl graph query social "MATCH (n) RETURN n LIMIT 5"
//	wavespanctl -k 3 vec search docs 0.1,0.2,0.3
//	wavespanctl backup begin
//	wavespanctl members
//	wavespanctl snapshot dump --storage ./data --out backup.wsb   (offline; run against a stopped node)
package main
...
import (
	...
	wavespan "github.com/yannick/wavespan/sdk/go"
)
...
var (
	...
	admin   = flag.String("admin", "localhost:7900", "node admin-port address host:port (members)")
...
func main() {
	...
	// Commands that do NOT talk to a node's data port are handled first: version is local; members hits
	// the admin HTTP port; snapshot operates offline on a wavesdb storage directory.
	switch args[0] {
	case "version":
	...
	default:
		die("unknown group %q (try: kv set hash zset coll graph vec budget lease backup | members version)", group)
	}
}
...
func die(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "wavespanctl: "+format+"\n", args...)
	os.Exit(1)
}

// membersCmd prints the cluster membership from the node's admin HTTP endpoint.
func membersCmd() {
	resp, err := http.Get("http://" + *admin + "/admin/membership")
	if err != nil {
		die("members: %v", err)
	}
...
func usage() {
	fmt.Fprint(os.Stderr, `wavespanctl — WaveSpan admin/client CLI (built on the Go SDK)

usage: wavespanctl [global-flags] <group> <op> [args...]

data-plane groups & ops (talk to -addr, default localhost:7800):
...
other commands:
  members                      cluster membership (admin port, -admin, default localhost:7900)
  version                      print the client version

(offline wavesdb storage dump/restore lives in the separate "wavespan-snapshot" binary — it links the
server storage engine, which cannot coexist with the SDK's proto stubs in one binary.)
