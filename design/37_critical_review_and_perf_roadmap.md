...
4. Concurrent + batched replication fan-out in `replicateMinAck` and `Fanout` (port the `proposer.go` coalescing pattern). (A1)
   **RESOLVED 2026-07-02:** new `StoreReplicaBatch` RPC + per-destination coalescing batcher (`replication/local/batcher.go`, 200µs window / 128 ops, group-commit-leader pattern) installed *inside* `ConnectReplicator.StoreReplica`, so coordinator min-ack writes, fanout fills, and repair pushes all batch transparently; receiver applies batch entries concurrently (16 workers) so the WAL group commit actually merges fsyncs; Unimplemented → unary replay for mixed-version clusters. `replicateMinAck` now fires the first minAck candidates concurrently (placement order preserved for fallbacks). Measured (Docker-for-Mac profile cluster): syncNone p99 get 3.3→2.3ms, get p50 826→641µs, puts ~flat on loopback (gains land on real RTT); syncFull put p99 140→79ms and throughput now scales with concurrency (852/s@32 → 1,270/s@128) — the remaining p50 floor is this host's 2×fsync, Linux measurement still owed. Race-clean; mixed-version + cancel + coalescing pinned by `batcher_test.go`.
5. Merged-iterator scans in `recordstore` (kills N+1; also speeds repair backfill + AE). (A2)
   **RESOLVED 2026-07-02 (via the inline-value alternative):** true meta/data co-iteration is impossible without an on-disk format change (meta keys are raw, data keys length-prefixed — different sort orders), so the winner's value (≤512B, `storage.InlineValueMax`) is now duplicated into the `LatestPointer` (`inline_value` field). Point Gets and range scans of small values are served from CFKVMeta alone — zero data-CF lookups (pinned by `countingStore` tests); large/legacy pointers fall back unchanged, tombstone/losing-apply/sibling/repair paths preserve or clear it correctly. `ScanRecords`/backfill/AE still read full records by design. Measured e2e: gets 20.8k→23.3k/s (p50 641→569µs, p99 2.3→1.9ms), puts 9.0k→10.0k/s. Trade-off: +1 small-value copy in the meta CF per live key (write amp, bounded at 512B).
6. gRPC tuning: windows, keepalive, buffer sizes; then mTLS parity with design/27. (A5)
   **RESOLVED 2026-07-02:** `rpcopts.GRPCConn` now dials with 1MiB/16MiB HTTP/2 windows, 20s/5s keepalive (PermitWithoutStream), 64MiB recv cap; `grpcsrv.New` mirrors them plus server keepalive + enforcement policy. mTLS parity: `rpcopts.ConfigureDialTLS(tlsCfg.ClientTLS())` wired in the node when servers run mTLS (previously data clients dialed plaintext and could not even connect to an mTLS server); loopback mutual-TLS proven by `TestGRPCConnMTLS`, late-install misuse guarded by panic. E2e smoke on the profile cluster: identical throughput on loopback (20.3k gets/s, 8.8k puts/s), p99 slightly better, 0 errs. design/27 updated with a gRPC-era status note.
...
10. Incremental manifest. (B4)
    **RESOLVED 2026-07-02 (format-preserving; wavesdb `3e1a078`):** the catalog snapshot is now taken under `db.mu` (cheap, in-memory, generation-stamped) and the O(total-sstables) encode + fsync + rename happens OUTSIDE it, serialized by a dedicated `manifestMu` with newer-generation supersession — the global lock is no longer held across manifest I/O on any flush/compaction. The full RocksDB-style edit log is deliberately deferred: the backup/objstore layer (checkpoint_objstore, waveSpan restore_bootstrap) installs `MANIFEST` blobs verbatim, so a format change must first rework that contract. Pinned by `TestManifestSurvivesConcurrentStructuralChurn`.
